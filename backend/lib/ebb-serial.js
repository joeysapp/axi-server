...
export class EBBSerial {
	...
	async query(cmd, timeoutOverride = null) {
		...
		const noOKCommands = ['A', 'I', 'MR', 'PI', 'QM', 'QG', 'QS', 'V'];
	...
	/**
		* Query Utility (QU command) - requires v3.0+
		* @param {number} param - Parameter number (1-6, 60-61, 200)
		* @returns {Promise<string>}
		*/
	async queryUtility(param) {
		const response = await this.query(`QU,${param}`);
		// The response is usually 'QU,Value' in future mode or 'Value' in legacy
		if (response.startsWith('QU,')) {
