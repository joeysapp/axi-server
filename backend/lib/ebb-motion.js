...
export class EBBMotion {
	...
	/**
		* Move to absolute position using HM command if available, or SM
		* @param {number} x - Target X in steps
		* @param {number} y - Target Y in steps
		* @param {number} rate - Step frequency (2-25000 steps/sec)
		*/
	async moveToAbsolute(x, y, rate = 2000) {
		// HM command: HM,rate,pos1,pos2 - moves to absolute position
		// AxiDraw CoreXY: pos1 = x+y, pos2 = x-y
		const pos1 = Math.round(x + y);
		const pos2 = Math.round(x - y);

		await this.ebb.command(`HM,${rate},${pos1},${pos2}`);

		// Position is now target
		this.posX = Math.round(x);
		this.posY = Math.round(y);

		return 0; // HM returns immediately
	}

	/**
		* Reset the zero point by disabling and enabling motors
		*/
	async zero() {
		const currentRes = this.motorResolution || MOTOR_RESOLUTION.MICRO_16X;
		await this.disableMotors();
		await this.enableMotors(currentRes);
		this.posX = 0;
		this.posY = 0;
	}
	...
	/**
		...
		* @param {boolean} force - If true, skip bounds clamping (allows negative movement)
		*/
	async moveXY(deltaX, deltaY, duration = null, force = false) {
	...
	/**
		...
		* @param {number} durationOverride - Duration in ms (optional)
		* @param {boolean} force - Skip bounds clamping (optional)
		*/
	async moveXYInches(inchesX, inchesY, speed = null, durationOverride = null, force = false) {
		...
		return await this.moveXY(stepsX, stepsY, duration, force);
	}
	...
	/**
		...
		* @param {boolean} force - Skip bounds clamping (optional)
		*/
	async moveXYMm(mmX, mmY, speed = null, durationOverride = null, force = false) {
		...
		return await this.moveXYInches(inchesX, inchesY, speedInches, durationOverride, force);
	}
