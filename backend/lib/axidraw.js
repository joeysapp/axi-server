...
export class AxiDraw {
	...
	/**
		...
		* @param {object} options - { rate, force }
		*/
	async moveTo(x, y, units = 'mm', options = {}) {
		await this.ensureReady();
		this._setState(AxiDrawState.BUSY);
		...
		const duration = await this.motion.moveToAbsolute(stepsX, stepsY, options.rate);
		this._setState(AxiDrawState.READY);
		this._logAction('move_to', { x, y, units, ...options });
		return duration;
	}
	...
	/**
		...
		* @param {string} units - 'steps', 'inches', or 'mm' (default: 'mm')
		* @param {object} options - { speed, duration, force }
		*/
	async move(dx, dy, units = 'mm', options = {}) {
		...
		switch (units) {
			case 'inches':
				duration = await this.motion.moveXYInches(dx, dy, options.speed, options.duration, options.force);
				break;
			case 'mm':
				duration = await this.motion.moveXYMm(dx, dy, options.speed, options.duration, options.force);
				break;
			default:
				duration = await this.motion.moveXY(dx, dy, options.duration, options.force);
		}

		this._setState(AxiDrawState.READY);
		this._logAction('move', { dx, dy, units, ...options });
		return duration;
	}
	...
	async lineTo(dx, dy, units = 'mm', options = {}) {
		...
		switch (units) {
			case 'inches':
				duration = await this.motion.moveXYInches(dx, dy, options.speed || this.motion.speedPenDown, options.duration, options.force);
				break;
			case 'mm':
				duration = await this.motion.moveXYMm(dx, dy, options.speed || (this.motion.speedPenDown * 25.4), options.duration, options.force);
				break;
			default:
				duration = await this.motion.moveXY(dx, dy, options.duration, options.force);
		}
		...
		this._setState(AxiDrawState.READY);
		this._logAction('line_to', { dx, dy, units, ...options });
		return duration;
	}
	...
	/**
		* Reset zero point (logical home) by disabling and re-enabling motors
		*/
	async zero() {
		await this.ensureReady();
		await this.motion.zero();
		this._logAction('zero');
	}
	...
	async getStatus(queryHardware = true) {
		...
		if (this.ebb && this.ebb.connected) {
			...
			if (queryHardware) {
				...
				try {
					status.hardware = await this.ebb.queryGeneral();
				} catch (e) {
					// Ignore query errors
				}
